...
function Ocean:NewWindow()
	...
	local Main = createInstance("Frame", {
		Name = "Main",
		Position = UDim2.fromScale(0.248, 0.22),
	...
	local Divider1 = createInstance("Frame", {
		Name = "Divider1",
		Position = UDim2.fromScale(0.311, 0),
	...
	local Divider1_1 = createInstance("Frame", {
		Name = "Divider1_1",
		Position = UDim2.fromScale(0.311, 0.13),
		Size = UDim2.new(0.689, 0, 0, 2),
		Parent = Main,
		BackgroundColor3 = Color3.fromRGB(82, 81, 81),
		BorderSizePixel = 0,
		ZIndex = 2
	...
	local TabHolder = createInstance("ScrollingFrame", {
		...
		Size = UDim2.fromScale(0.311, 0.8),
		Parent = Main,
	...
	local Title = createInstance("TextLabel", {
		Name = "Title",
		Position = UDim2.fromScale(0.029, 0.03),
	...
	function Window:Tab(Title: string, ImageID: string)
		...
		local Icon = createInstance("ImageLabel", {
			Name = "Icon",
			Position = UDim2.fromScale(0.044, 0.2),
		...
		local Canvas = createInstance("ScrollingFrame", {
			Name = "Canvas",
			Position = UDim2.fromScale(0.311, 0.15),
			Size = UDim2.fromScale(0.689, 0.85),
			Parent = Main,
			BackgroundTransparency = 1,
			ScrollBarThickness = 0,
			ClipsDescendants = true,
		...
		local Path = createInstance("TextLabel", {
			Name = "Path",
			Position = UDim2.fromScale(0.35, 0.045),
		...
		function tab:Activate()
			...
			TS:Create(Tab, TweenInfo.new(0.1, Enum.EasingStyle.Quad), {BackgroundTransparency =0.5}):Play()
		...
		function tab:Toggle(Title: string, Options: table)
			...
			function Settings:Toggle(v)
				...
				if Settings.Value then
					...
					TS:Create(Knob_17, TweenInfo.new(0.1, Enum.EasingStyle.Sine), {Position = UDim2.fromScale(0.55, 0.5)}):Play()
				else
					...
					TS:Create(Knob_17, TweenInfo.new(0.1, Enum.EasingStyle.Sine), {Position = UDim2.fromScale(-0.15, 0.5)}):Play()
				end
			end
		...
		function tab:Slider(Title: string, Options: table)
			...
			local SliderAmount = createInstance("TextBox", {
				Name = "SliderAmount",
				Position = UDim2.new(0.88, 0, 0.3, 0),
			...
			function Settings:SetValue(v)
				if not v then
					...
					TS:Create(SliderMain, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {Size = UDim2.fromScale((v - Settings.Min) / (Settings.Max - Settings.Min), 1)}):Play()
        ...
        if on then
            for Index, Object in next, workspace:GetChildren() do
                    ...
                    if Object:IsA("BasePart") and Object.Name == "Football" then
						if getgenv().msVersion == "Magnets V1" then
							local Distance = (HumanoidRootPart.Position - Object.Position).Magnitude
								if string.find(getexecutorname(), "Solara") or string.find(getexecutorname(), "Wind") or string.find(getexecutorname(), "Zorara")  or string.find(getexecutorname(), "Codex")  or string.find(getexecutorname(), "Cryptic") then
...
----------------------------------------------------/-/-/-/-//-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/////////////////-/-/-/-/-/-/-							
	...
	local function getPeopleGuardingClosestToMouse(WR)
		local ClosestCB, MaxDistance = nil, 9000000000
		if game.PlaceId == 8206123457 then
